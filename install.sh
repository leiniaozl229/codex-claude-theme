#!/bin/bash
# Creates a separately signed, locally themed Codex copy on macOS.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: ./install.sh [--source /path/to/ChatGPT.app] [--target /path/to/Lab.app] [--force]

Copies the installed Codex app, applies the Claude-style visual layer, and
ad-hoc signs the copy. The original app is never modified.
EOF
}
