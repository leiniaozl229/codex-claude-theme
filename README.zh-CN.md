...
请按上面的命令启动副本，并保留独立的 `--user-data-dir`。这样，副本与正式版的本地设置、缓存、窗口状态和本地任务记录会彼此隔离；首次使用主题副本时通常需要重新登录。

登录同一个账号后，账号侧会同步的内容可以在副本中再次显示；但不要假定每一条保存在本机的 Codex 任务或偏好都会在两端自动同步。正式版继续使用原有的 `~/Library/Application Support/Codex` 数据。

也可以去掉 `--user-data-dir`，让副本尝试共用正式版的本地状态；但不建议这样做：两个客户端可能同时争用同一批文件，而且经过本机 ad-hoc 签名的副本可能额外触发 Keychain 访问提示。
